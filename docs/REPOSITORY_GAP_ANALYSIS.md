# REPOSITORY GAP ANALYSIS — VESPER
...
> This is a deliberately critical audit of the VESPER document set, written before Phase I executes so that defects are cheap to fix. It hunts for contradictions, missing assumptions, undefined terms, missing benchmark requirements, and reviewer-facing weaknesses. Findings are severity-ranked and cross-referenced. Nothing here changes the science; it identifies where the science is not yet pinned down.
...
| Path | Lines | Role |
|------|-------|------|
| `docs/VESPER.md` | 340 | Master charter (author: Ansul Suryawanshi) |
| `docs/VESPER_CONCEPT_RECONSTRUCTION.md` | 154 | Concept lineage / v3 post-mortem |
| `docs/VESPER_ARCHITECTURE.md` | 196 | 7-stage system design |
| `docs/VESPER_PHASE1_VALIDATION.md` | 122 | Pre-registered Phase I protocol |
| `docs/SCIENTIFIC_HYPOTHESIS.md` | 146 | Formal hypotheses |
| `docs/VESPER_MATHEMATICAL_FOUNDATIONS.md` | 255 | Canonical theory |
...
| `VESPER.md`, `README.md`, `CLAUDE.md` (root) | 0 | **Empty** |
...
| **F12** | Repo hygiene: empty root `VESPER.md` duplicate, empty `README.md`/`CLAUDE.md`, no bibliography, no data-version pinning | **Low** | Hygiene |
...
- **Exchangeability for the bootstrap null (F10).** `VESPER_MATHEMATICAL_FOUNDATIONS.md` §9 notes red noise breaks strict exchangeability and "motivates block resampling" but commits to nothing — leaving the FAP's validity assumption open.
...
- **z⋆, θ, N_min, "strong event."** The detection threshold and routing-rule parameters are symbolic throughout (`VESPER_PHASE1_VALIDATION.md` §2 lists the rule structure but no values, no target false-alarm rate they are set to on the calibration set).
...
- **"Compute / total cost" aggregation.** `VESPER_PHASE1_VALIDATION.md` §4 defines the unit (CPU-core-seconds) but not the aggregation (sum vs. median per star) nor whether the shared conditioning cost is included (it should be excluded or charged equally to both arms).
...
- **Power analysis shown.** `VESPER_PHASE1_VALIDATION.md` §6 asserts ≥500 injections/cell "from a power analysis"; the analysis (assumed baseline recall, α, power, resulting N) must be exhibited.
...
- **Empty duplicate `VESPER.md` at repo root** (the real one is `docs/VESPER.md`) — delete or make it a pointer to avoid an authoritative-source ambiguity.
...
1. **F1** — define the compute-measurement population and either scope H1b or add a clean-skip tier (decide explicitly). *Update `VESPER_PHASE1_VALIDATION.md` §4 + `SCIENTIFIC_HYPOTHESIS.md` H1b + `VESPER_MATHEMATICAL_FOUNDATIONS.md` §8.*
2. **F2** — write the operational recovery/recall predicate (period tolerance, harmonics, epoch, SDE). *Add to `VESPER_PHASE1_VALIDATION.md` §4.*
3. **F6** — specify a single primary estimand + multiplicity rule for non-inferiority. *`SCIENTIFIC_HYPOTHESIS.md` §6.*
4. **F8 + §4 baseline config** — pin z⋆/θ/T, the TLS configuration, and the runtime protocol. *`VESPER_PHASE1_VALIDATION.md` §2–§4.*

**Should-fix (before publication, ideally before run):**
5. **F4/F5/F3** — state single-planet & strict-periodicity assumptions; add a monotransit vetting criterion; scope the model-agnostic claim. *`SCIENTIFIC_HYPOTHESIS.md` §5, `VESPER_MATHEMATICAL_FOUNDATIONS.md` §4/§6.*
6. **Required ablation + null-star FP target + power analysis shown.** *`VESPER_PHASE1_VALIDATION.md` §5–§6.*
