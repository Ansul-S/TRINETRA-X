...
Next Milestone:
**PHASE I COMPLETE.** M4 (H1 falsified — compute branch) + M5/M6 characterization + M7 write-up all done and merged to `main` (PRs #9–#13). Manuscript `papers/phase1_evidence_first_triage.md` (author **Ansul Suryawanshi**); T1–T8, F3–F8; `references.bib` (8 core refs ADS-verified). Open Phase-I polish (optional): venue (AJ/MNRAS), F1/F4/F9.
**NEXT DIRECTION — PHASE II: Kepler scaling experiment** (a new, separately pre-registered experiment, P-8; Phase I stays sealed/final, no v4). Tests the central thesis the TESS result could not: that the evidence-first compute advantage **scales with search-space size** (fast lane folds k events vs TLS folds N points per period; k≪N). Kepler = dense, ~4-yr baseline, large period grid, available now (LSST is out of scope — sparse cadence). Sketch: `docs/PHASE2_KEPLER_SCALING_PREREG.md`.

Next Action:
**Owner: sign off the Kepler-experiment decisions D1–D5** (cadence, target sample, baseline-truncation set, H1ᴋ margin, compute budget) in the pre-reg sketch → then promote to a full Phase-II pre-registration and proceed milestone-by-milestone. **Nothing sealed or run until that pre-registration is signed.** Deferred to their own separate experiments: Lever 3 (calibrated clean-skip — the survey-scale lever), cross-domain generalization (only vs correct incumbents, periodic-search domains).
